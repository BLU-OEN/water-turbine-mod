...
for(const p of walk(pack).filter(p=>p.endsWith('.png'))){const {w,h,raw}=pngInfo(p);for(let y=0;y<h;y++)for(let x=0;x<w;x++){const i=y*(w*4+1)+1+x*4;if(raw[i+3]){assert.equal(raw[i],raw[i+1],`Non-neutral pixel in ${p}`);assert.equal(raw[i+1],raw[i+2],`Non-neutral pixel in ${p}`);}}}
...
console.log(`PASS ${kind}: model references, UVs, neutral monochrome textures, ${turbine?'eight facing/water states and animation':'armor head UVs and transparency'}, offline preview.`);
